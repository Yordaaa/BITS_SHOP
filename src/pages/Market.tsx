...
const filterOptions = [
    {
        id: 'categories',
        label: 'Categories',
        options: ['Laptops', 'Smartphones', 'Tablets', 'Cameras', 'Accessories', 'Others']
    },
    {
        id: 'brand',
        label: 'Brand',
    ...
    },
    {
        id: 'features',
        label: 'Features',
        options: ['4G Connectivity', '5G Connectivity', 'WiFi 6 Support', 'NFC Support']
    }
];
                        ...
                        <div className="fixed inset-0 z-40 flex">
                            <DialogPanel className="relative ml-auto h-full w-full max-w-xs flex flex-col bg-white py-4 pb-12 shadow-xl">
                                <div className="flex justify-between px-4">
                                    ...
                                    <button onClick={() => setMobileFiltersOpen(false)} className="h-10 w-10 rounded-md p-2 text-gray-400">
                                ...
                                <Menu as="div" className="relative text-left">
                                    ...
                                    <MenuItems className="absolute right-0 mt-2 w-40 origin-top-right rounded-md bg-white shadow-2xl ring-1 ring-black ring-opacity-5">
                                        {sortOptions.map(({ name, href, current }) => (
                                            <MenuItem key={name}>
                                                <a href={href} className={`block px-4 py-2 text-sm ${current ? 'font-medium text-gray-900' : 'text-gray-500'} hover:bg-gray-100`}>
                                                    {name}
                                                </a>
                                            </MenuItem>
                                        ))}
                                            ...
                                            <DisclosurePanel className="pt-6">
                                                <div className="space-y-4">
                                                    {options.map((option, index) => (
                                                        <div key={option} className="flex items-center">
                                                            <input type="checkbox" className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                                                            <label htmlFor={`filter-${id}-${index}`} className="ml-3 text-sm text-gray-600">
                                                                {option}
                                                            </label>
                                                        </div>
                                                    ))}
